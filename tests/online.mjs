...
const proxyMode = process.env.MOE_ONLINE_PROXY_MODE || 'none'
if (!['none', 'system'].includes(proxyMode)) throw new Error('MOE_ONLINE_PROXY_MODE 仅支持 none 或 system')
await writeFile(join(profile, 'network.json'), JSON.stringify({ settings: { globalMode: proxyMode, proxyAddress: '127.0.0.1:1080', siteModes: { 'konachan-g': 'default', pixiv: 'default' } }, pixivVerified: false }))
