...
import { validSite } from './network.ts'
...
export function validateSearch(value: unknown): SearchInput {
  ...
  if (v.site !== undefined && !validSite(v.site)) throw new Error('站点无效')
  if (v.customCategory !== undefined && (!Number.isInteger(v.customCategory) || v.customCategory < 0 || v.customCategory > 99)) throw new Error('分类无效')
  if (v.site === 'pixiv') {
  ...
  return { ...(v.customCategory !== undefined ? {customCategory: v.customCategory} : {}), ...(v.site ? { site: v.site } : {}), ...(v.site === 'pixiv' ? { pixivMode:v.pixivMode ?? 'tag', pixivKind:v.pixivKind ?? 'illust', pixivPeriod:v.pixivPeriod ?? 'daily', pixivDate:v.pixivDate ?? '' } : {}), keyword: v.keyword, page: v.page, count: v.count, filterResolution: v.filterResolution, minWidth: v.minWidth, minHeight: v.minHeight, orientation: v.orientation }
}
export function query(input: SearchInput, page = input.page): string {
  if (input.site === 'danbooru') return `https://danbooru.donmai.us/posts.json?${new URLSearchParams({page:String(page),limit:String(input.count),tags:`${input.keyword} rating:general`})}`
  if (input.site === 'gelbooru') return `https://gelbooru.com/index.php?${new URLSearchParams({page:'dapi',s:'post',q:'index',pid:String(page-1),limit:String(input.count),tags:`${input.keyword} rating:general`})}`
  if (input.site === 'yande') return `https://yande.re/post.xml?${new URLSearchParams({page:String(page),limit:String(input.count),tags:`${input.keyword} rating:s`})}`
...
export function parsePictures(value: unknown, input: SearchInput, viewed: Viewed): Picture[] {
  ...
  return value.map(raw => {
    ...
    const danbooru = input.site === 'danbooru'
    const width = integer(danbooru ? r.image_width : r.width), height = integer(danbooru ? r.image_height : r.height), id = integer(r.id)
    if (id <= 0) throw new Error('图片 ID 无效')
    const safebooru = input.site === 'safebooru', yande = input.site === 'yande', gelbooru = input.site === 'gelbooru'
    const siteHome = yande ? 'https://yande.re' : gelbooru ? 'https://gelbooru.com' : home
    const nsfw = safebooru ? false : gelbooru ? text(r.rating) !== 'general' : danbooru ? text(r.rating) !== 'g' : text(r.rating) !== 's'
    ...
    const tags = text(danbooru ? r.tag_string : r.tags).split(' ')
    while (tags.length && !tags[0].trim()) tags.shift()
    return {
      key: '', id, width, height, score: integer(r.score), author: text(r.author ?? r.uploader_name), authorId: text(danbooru ? r.uploader_id : r.creator_id), tags: tags.map(t => t.trim()), date,
      source: text(r.source), detail: safebooru || gelbooru ? `${siteHome}/index.php?page=post&s=view&id=${id}` : danbooru ? `https://danbooru.donmai.us/posts/${id}` : `${siteHome}/post/show/${id}`, thumbnail: text(safebooru ? text(r.preview_url).replace(/\.(png|jpeg)(?=\?|$)/i, '.jpg') : danbooru ? r.preview_file_url : r.preview_url), ...(safebooru || yande || gelbooru ? { large: text(r.jpeg_url || r.sample_url || r.file_url) } : {}), preview: text(danbooru ? r.large_file_url || r.file_url : safebooru || yande || gelbooru ? r.sample_url || r.file_url : r.sample_url), original: text(r.file_url), bytes: Math.max(0, integer(r.file_size)), nsfw,
      ...(danbooru ? { artist:text(r.tag_string_artist), character:text(r.tag_string_character), copyright:text(r.tag_string_copyright), detailsLoaded:true } : {}),
