...
import { parseDanbooruHints } from './danbooru'
...
export function installBrowser(main: () => BrowserWindow | undefined, createPreview: (key: string) => BrowserWindow): BrowserState {
  ...
  handle('hints', async (_e, request) => {
    ...
    if (site === 'danbooru') return parseDanbooruHints(await getText(`${sites.danbooru.home}/autocomplete?${new URLSearchParams({'search[query]':keyword,'search[type]':'tag_query',version:'1',limit:'20'})}`, signal, site))
